...
import { useVentures, useAllProfiles } from '../../hooks/useVentures';
...
export function AdminDashboard() {
  ...
  const tierCounts = {
    prime: (ventures || []).filter((v) => v.tier === 'prime').length,
    core: (ventures || []).filter((v) => v.tier === 'core').length,
    select: (ventures || []).filter((v) => v.tier === 'select').length,
  };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{venturesLoading ? '—' : totalVentures}</p>
        ...
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{profilesLoading ? '—' : totalUsers}</p>
        ...
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-green-600">{activeVentures}</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left text-xs font-medium text-gray-500 pb-2">Joined</th>
                </tr>
              </thead>
              <tbody>
                {(profiles || []).slice(0, 20).map((user) => (
                  <tr key={user.id} className="border-b border-gray-100">
                    ...
                    <td className="py-3"><Badge>{ROLE_LABELS[user.role] || user.role}</Badge></td>
                    <td className="py-3 text-sm text-gray-600">{user.organization || '—'}</td>
        ...
        <Card>
          <div className="text-center">
            <div className="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center mx-auto mb-2">
              <span className="text-lg font-bold text-purple-700">{tierCounts.prime}</span>
            </div>
        ...
        <Card>
          <div className="text-center">
            <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center mx-auto mb-2">
              <span className="text-lg font-bold text-blue-700">{tierCounts.core}</span>
            </div>
        ...
        <Card>
          <div className="text-center">
            <div className="w-12 h-12 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-2">
              <span className="text-lg font-bold text-amber-700">{tierCounts.select}</span>
            </div>
